 ...
 */
public class CodeGenerator_vue {
    /**
     * 主程序入口
     */
    public static void main(String[] args) {
        genCode("sys_about_us");
...
//        name.forEach(item->System.out.println(item.getName()));
    ...
    public static String gainTableName(String tableName) {
        try {
            ...
            //3.通过数据库的连接操作数据库，实现增删改查（使用Statement类）
            Statement st=conn.createStatement();
            ResultSet rs=st.executeQuery("SELECT table_name ,table_comment FROM information_schema.TABLES WHERE table_schema = 'go'and TABLE_NAME ='"+tableName+"'"+"");
            //4.处理数据库的返回结果(使用ResultSet类)
...
//        genVueController(tableName, modelName);
...
//        getMapper(tableName, modelName);//获取Mapper
    ...
    private static void getVueTable(String tableName, String modelName) {
        try {
            ...
            System.out.println("<<<"+modelNameUpperCamel);
            File file=new File(PROJECT_PATH+RESOURCES_PATH+PACKAGE_PATH_VUE_LIST+modelNameUpperCamel+".vue");
    ...
    private static void getVueFrom(String tableName, String modelName) {
        try {
            ...
            final String remarks=gainTableName(tableName);//表的备注
            data.put("remarks", remarks);
            File file=new File(PROJECT_PATH+RESOURCES_PATH+PACKAGE_PATH_VUE_LIST+modelNameUpperCamel+"Form"+".vue");
    ...
    private static void genVueController(String tableName, String modelName) {
        try {
            freemarker.template.Configuration cfg=getConfiguration();
            Map<String, Object> data=new HashMap<>();
            data.put("date", DATE);
            ...
            final String remarks=gainTableName(tableName);//表的备注
            data.put("remarks", remarks);
            File file=new File(PROJECT_PATH+JAVA_PATH+PACKAGE_PATH_CONTROLLER+"console/"+"Vue"+modelNameUpperCamel+"Controller.java");
            if (!file.getParentFile().exists()) {
                file.getParentFile().mkdirs();
            }
            cfg.getTemplate("controller-vue.ftl").process(data, new FileWriter(file));
            System.out.println(modelNameUpperCamel+"Controller.java 生成成功");
        } catch (Exception e) {
            throw new RuntimeException("生成Controller失败", e);
        }
